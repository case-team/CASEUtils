...
import os
import h5py
from ROOT import gROOT, TH1F, TGraph, TGraphAsymmErrors, TCanvas, TLegend, TLatex, Double, TFile
...
def trigger_hist_fill(hist_all, hist_pass, ntuple_file_info, x_var="mjj"):
    ...
    if x_var != "mjj":
        if x_var.endswith("1"):
            var_index = VARIABLES[x_var.replace("1","")][0]
        elif x_var.endswith("2"):
            var_index = VARIABLES[x_var.replace("2","")][1]
        else:
            var_index = VARIABLES[x_var][0]
        fill_hist(hist_all, preselection_vars[preselection_mask][:,var_index])
        fill_hist(hist_pass, preselection_vars[(preselection_mask)&(trigger_mask)][:,var_index])
    else:
        fill_hist(hist_all, trigger_vars[preselection_mask][:,0])
        fill_hist(hist_pass, trigger_vars[(preselection_mask)&(trigger_mask)][:,0])
...
def trigger_efficiency_line(data_path, base_dataset, year, output_file, x_var="mjj"):

    print("preselections:", PRESELECTIONS)
    print("\ntriggers:", TRIGGER_YEARS)

    bin_array = array("d", BINNING)
    hist_pass = TH1F("pass_{}_{}".format(base_dataset, year),
        "pass_{}_{}".format(base_dataset, year), len(bin_array)-1, bin_array)
    hist_all = TH1F("all_{}_{}".format(base_dataset, year),
        "all_{}_{}".format(base_dataset, year), len(bin_array)-1, bin_array)
    ...
    for i, ntuple_file in enumerate(file_list):
        ...
        trigger_hist_fill(hist_all, hist_pass, ntuple_file, x_var=x_var)

    # Make efficiency graph
    hist_pass.Sumw2()
    hist_all.Sumw2()
    eff = TGraphAsymmErrors()
    eff.Divide(hist_pass, hist_all)
    print_hist_content(hist_all)
    return eff, (hist_pass, hist_all)


def combined_efficiency(hist_pass_list, hist_all_list):
...
def trigger_efficiency_plot(line_dict, year, output_file, x_var="mjj"):
 
    # Plotting
    for line_title in line_dict.keys():
        line_dict[line_title]["graph"].SetMarkerColor(line_dict[line_title]["color"])
        line_dict[line_title]["graph"].SetMarkerStyle(8)
        line_dict[line_title]["graph"].SetLineColor(line_dict[line_title]["color"])
        line_dict[line_title]["graph"].SetLineWidth(2)
    ...
    c1 = TCanvas("c1", "Trigger Efficiency", 800, 600)
    #c1.SetGrid()
    c1.cd(1)

    if x_var != "mjj":
        if x_var =="pt1":
            x_label = "leading jet p_{T}"
        elif x_var =="pt2":
            x_label = "subleading jet p_{T}"
    else:
        x_label = "m_{jj} (GeV)"

    for i, line_title in enumerate(line_dict.keys()):
        if i==0:
            line_dict[line_title]["graph"].Draw("AP")
            line_dict[line_title]["graph"].SetTitle(";{};trigger efficiency".format(x_label))
            line_dict[line_title]["graph"].SetMinimum(0.)
            line_dict[line_title]["graph"].SetMaximum(1.4)

            line_dict[line_title]["graph"].GetXaxis().SetTitleSize(0.045)
            line_dict[line_title]["graph"].GetYaxis().SetTitleSize(0.045)
            line_dict[line_title]["graph"].GetYaxis().SetTitleOffset(1.1)
            line_dict[line_title]["graph"].GetXaxis().SetTitleOffset(1.05)
            line_dict[line_title]["graph"].GetXaxis().SetLimits(BINNING[0], BINNING[-1])
        else:
            line_dict[line_title]["graph"].Draw("P SAME")

    one_line.Draw("L")
    c1.SetTopMargin(0.05)

    if len(line_dict.keys()) > 1:
        leg = TLegend(0.55, 0.75, 0.85, 0.9)
        leg.SetBorderSize(0)
        leg.SetFillStyle(0) #1001
        leg.SetFillColor(0)
        for i, line_title in enumerate(line_dict.keys()):
            leg.AddEntry(line_dict[line_title]["graph"], line_dict[line_title]["label"])
        leg.Draw()
...
def trigger_efficiency(data_path, base_dataset, year, output_file,
    combined=False, efficiency_threshold=0.99, x_var="mjj"):

    line_dict = OrderedDict()
    if year=="run2":
        print("producing all three years and overlaying them...")
        graph_2016, (hist_pass_2016, hist_all_2016) = trigger_efficiency_line(
            data_path, base_dataset, "2016", output_file, x_var=x_var)
        graph_2017, (hist_pass_2017, hist_all_2017) = trigger_efficiency_line(
            data_path, base_dataset, "2017", output_file, x_var=x_var)
        graph_2018, (hist_pass_2018, hist_all_2018) = trigger_efficiency_line(
            data_path, base_dataset, "2018", output_file, x_var=x_var)
        line_dict["2016"] = {"graph": graph_2016, "color": 4,"label": "2016 (35.9 fb^{-1})"}
        line_dict["2017"] = {"graph": graph_2017, "color": 6,"label": "2017 (41.5 fb^{-1})"}
        line_dict["2018"] = {"graph": graph_2018, "color": 8,"label": "2018 (59.7 fb^{-1})"}
        print("\n2016 turn-on:")
        print_threshold(graph_2016, efficiency_threshold)
        print("\n2017 turn-on:")
        print_threshold(graph_2017, efficiency_threshold)
        print("\n2018 turn-on:")
        print_threshold(graph_2018, efficiency_threshold)

        if combined:
            hist_pass_list = [hist_pass_2016, hist_pass_2017, hist_pass_2018]
            hist_all_list = [hist_all_2016, hist_all_2017, hist_all_2018]
            graph_run2, (hist_pass_combined, hist_all_combined) = combined_efficiency(hist_pass_list, hist_all_list)
            line_dict["run2"] = {"graph": graph_run2, "color": 1,"label": "combined (137 fb^{-1})"}
            print("\nrun2 turn-on:")
            print_threshold(graph_run2, efficiency_threshold)

    else:
        assert not combined, "cannot combine years if only 1 year is selected!"
        graph, (hist_pass_combined, hist_all_combined) = trigger_efficiency_line(data_path, base_dataset, year, output_file, x_var=x_var)
        line_dict[year] = {"graph": graph, "color": 2, "label": None}

    trigger_efficiency_plot(line_dict, year, output_file, x_var=x_var)
...
if __name__ == "__main__": 
    ...
    parser.add_argument("--combined",  dest="combined", default=False, action='store_true',
        help="Combine the run2 data-taking years.")
    ...
    parser.add_argument("--variable",  dest="variable", type=str,
        default="mjj", action='store', choices=["mjj", "pt1", "pt2"],
        help="Variable with respect to which the efficiency is drawn.")
    ...
    if args.variable != "mjj":
        if "pt" in args.variable:
            BINNING = [200, 210, 220, 230, 240, 250, 260, 270, 280, 290, 300,
    ...
    trigger_efficiency(args.data_path , args.base_dataset, args.year,
        args.output_file, combined=args.combined, x_var=args.variable)
