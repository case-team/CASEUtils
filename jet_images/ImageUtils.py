...
def pixelate(jet, npix=40, img_width=0.8, rotate = True, norm=True):
...
def make_image(jet, jet_conts, npix, img_width= 0.8, rotate = True, norm = True):
    jet_conts = convert_to_pt_eta_phi(jet, jet_conts)
    return np.squeeze(pixelate(jet_conts, npix = 40, img_width = img_width, norm = True))
